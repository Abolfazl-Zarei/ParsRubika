...
// makeRequest یک درخواست HTTP به API روبیکا ارسال کرده و پاسخ را پارس می‌کند
// این نسخه اصلاح‌شده، پاسخ‌های با و بدون کلید "data" را مدیریت می‌کند.
func (c *Client) makeRequest(ctx context.Context, method string, apiMethod string, reqBody, respBody any) error {
	...
	if respBody != nil {
		...
		// ابتدا تلاش می‌کنیم پاسخ را به عنوان یک آبجکت JSON با کلید "data" پارس کنیم
		var rawResp map[string]json.RawMessage
		if err := json.Unmarshal(respData, &rawResp); err == nil {
			if data, ok := rawResp["data"]; ok {
				// اگر کلید "data" وجود داشت، از محتوای آن استفاده می‌کنیم
				dataToUnmarshal = data
			} else {
				// اگر کلید "data" وجود نداشت، کل پاسخ را در نظر می‌گیریم
				dataToUnmarshal = respData
			}
		} else {
			// اگر پاسخ اصلاً یک آبجکت JSON نبود، کل پاسخ را در نظر می‌گیریم
			dataToUnmarshal = respData
		}

		// در نهایت، داده‌های استخراج‌شده را در ساختار مورد نظر پارس می‌کنیم
		if err := json.Unmarshal(dataToUnmarshal, respBody); err != nil {
