...
func NewClient(token string, opts ...ClientOption) *BotClient {
	// ابتدا نمونه کلاینت را بدون فیلدهای وابسته به خودش ایجاد می‌کنیم
	client := &BotClient{
		token:             token,
	...
	// سپس گزینه‌های کاربر را اعمال می‌کنیم
	for _, opt := range opts {
		opt(client)
	}

	// در نهایت، مدیرانی که به نمونه کامل کلاینت نیاز دارند را ایجاد و مقداردهی می‌کنیم
...
// ... (بقیه متدهای فایل client.go در ادامه قرار می‌گیرند)
// متدهایی مانند Start, Stop, SendMessage, GetMe و غیره باید در اینجا قرار گیرند.
// برای کوتاه نگه داشتن پاسخ، فقط بخش اصلاح‌شده نمایش داده می‌شود.
...
func (c *BotClient) Stop(ctx context.Context) error {
	...
	c.isRunning = false
	close(c.stopChan)
	c.stopChan = make(chan struct{}) // ایجاد کانال جدید برای استفاده‌های آینده
	c.mu.Unlock()
...
func (c *BotClient) GetBotInfo(ctx context.Context) (*Bot, error) {
	// این متد ممکن است در API رسمی وجود نداشته باشد
	return c.GetMe(ctx)
}
