...
func (c *BotClient) StartWebhook(ctx context.Context, opts WebhookOptions) error {
	...
	server := &http.Server{
		...
		Handler:      mux,
		ReadTimeout:  15 * time.Second,  // محدودیت زمانی برای خواندن درخواست
		WriteTimeout: 15 * time.Second,  // محدودیت زمانی برای ارسال پاسخ
		IdleTimeout:  120 * time.Second, // مدت زمانی که اتصال باز می‌ماند
	}
...
func (c *BotClient) webhookHandler(opts WebhookOptions) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// بررسی secret اگر تنظیم شده باشد (لایه امنیتی)
		if opts.Secret != "" {
		...
		// محدود کردن حجم بدنه درخواست برای جلوگیری از حملات (حداکثر 1 مگابایت)
		...
		if err := opts.Handler(r.Context(), update); err != nil {
			...
			// ارسال خطای داخلی سرور به کلاینت
			http.Error(w, "خطا در پردازش", http.StatusInternalServerError)
			return
		}

		// ارسال پاسخ موفقیت‌آمیز به سرور روبیکا
...
func (c *BotClient) convertInlineToUpdate(inlineMsg *InlineMessage) *Update {
	return &Update{
		...
		NewMessage: &Message{
			// مستقیماً از رشته استفاده می‌کنیم، نیازی به تبدیل به int64 نیست
